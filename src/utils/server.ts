...
export class A11yChecklistServer {
  ...
  setupRoutes(router: Router): void {
        ...
        try {
          ...
          if (!componentId) {
            return res.status(400).json({
              error: "componentId parameter is required",
            });
          }
          ...
          if (!checklist) {
            ...
            isOutdated =
              await this.checklistManager.isChecklistOutdated(checklist);
          }

          // Get current component hash
          const hashResponse =
            await this.checklistManager.computeComponentHash(componentPath);
    ...
    // Save checklist for a component
    router.put(
      "/a11y-checklist/:componentId",
      async (req: Request, res: Response) => {
        try {
          ...
          if (payload.componentId !== componentId) {
            return res.status(400).json({
              error: "componentId in URL must match componentId in checklist",
            });
          }
    ...
    router.get(
      "/a11y-checklists/outdated",
      async (req: Request, res: Response) => {
        try {
          const outdatedChecklists =
            await this.checklistManager.getOutdatedChecklists();
    ...
    router.get(
      "/a11y-checklists/failing",
      async (req: Request, res: Response) => {
        try {
          const failingChecklists =
            await this.checklistManager.getFailingChecklists();
